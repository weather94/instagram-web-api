...
class Instagram {
  constructor(
    { username, password, cookieString },
  ...
  async getMediaFeedByLocation({ locationId, cursor }) {
    if (cursor) {
      return this.request('/graphql/query/', {
        qs: {
          query_hash: '36bd0f2bf5911908de389b8ceaa3be6d',
          variables: JSON.stringify({ id: locationId, first: 12, after: cursor })
        }
      }).then(data => {
  ...
  async getMediaFeedByHashtag({ hashtag, cursor }) {
    if (cursor) {
      return this.request('/graphql/query/', {
        qs: {
          query_hash: '9b498c08113f1e09617a1703c22b2f32',
          variables: JSON.stringify({ tag_name: hashtag, first: 12, after: cursor })
        }
      }).then(data => {
        return data.data.hashtag;
      })
    } else {
      return this.request(`/explore/tags/${encodeURIComponent(hashtag)}/?__a=1`).then(
